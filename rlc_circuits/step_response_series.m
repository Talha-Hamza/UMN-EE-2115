...
if alpha > omega_0
    ...
    syms t;
    v_t = VF + (A1_value * exp(s1 * t) + A2_value * exp(s2 * t));
...
elseif alpha < omega_0
    ...
    B1 = V0 - VF;
    B2 = ((I0/C) + (alpha * B1)) / omega_d;
    
    % Step 5: Construct v(t) for underdamped response
    syms t;
    v_t = (exp(-alpha * t) * (B1 * cos(omega_d * t) + B2 * sin(omega_d * t))) +VF;
